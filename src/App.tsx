...
function App() {
  ...
  // ✅ IMPROVED: Robust image preloading with loading state management
  useEffect(() => {
    const preloadRobotImages = async () => {
      console.log(`🖼️ Starting preload for robot: ${robotCharacterData.name}`);
      setImagesLoaded(false);
      
      // Get all pose image paths for the current robot
      const imagePaths = Object.values(robotCharacterData.poses);
      ...
      try {
        ...
        console.log(`🎉 All ${imagePaths.length} images loaded for ${robotCharacterData.name}`);
  ...
  // Update robotPose when robotCharacterData changes, but only after images are loaded
  useEffect(() => {
    if (imagesLoaded) {
      setRobotPose(robotCharacterData.poses.idle);
    }
  }, [robotCharacterData, imagesLoaded]);

  // Handle robot character change
  const handleSelectRobot = (robotId: string) => {
    ...
    const robotSpeeches = {
      ...
      melo: "Hey! I'm Melo, your musical friend. Ready to create some harmony in your thoughts and feelings?",
    ...
    // Only set celebration pose if images are loaded
    if (imagesLoaded) {
      setRobotPose(newRobot.poses.celebrate_both);
      
      // Return to idle after celebration using the new robot's idle pose
      setTimeout(() => {
